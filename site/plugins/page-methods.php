 ...
 *
 * Include following line in HTML head:
 * `$page->rel_prevnext($filter_key, $filter_value, $pagination, $page_num);`
 */
...
page::$methods['page_color'] = function($page) {

	if($page->project_color()->exists() && $page->project_color()->isNotEmpty()) {
		return $page->project_color()->html();
	}
	else {
		$random_project = site()->find('projects')->children()->visible()->shuffle()->first();

		if($random_project->project_color()->exists() && $random_project->project_color()->isNotEmpty()) {
			return $random_project->project_color()->html();
		}
	}
};
